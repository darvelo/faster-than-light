...
exports.getAll = function(req, res, next) {
  app.db.contexts.getContexts(req.user.id, function(err, contexts) {
    if (err) {
      return next(err);
    }
...
exports.oneById = function (req, res, next) {
  ...
  app.db.contexts.getContextById(req.user.id, req.params.id, function (err, context) {
    if (err) {
      return next(err);
...
exports.postContext = function (req, res, next) {
  ...
  // this requires the `expressValidator` middleware to be loaded.
  // not sure if I want to use it yet... but since this function isn't used yet i'll wait on it
