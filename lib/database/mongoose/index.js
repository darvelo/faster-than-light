  ...
  async.waterfall([
    ...
    function (callback) {
      exports.getUserByName(username, function (err, user) {
        ...
        user.lastContexts = ['516efbdc2d11144827000003'];
        bootstrap.user = user;
        callback(err, user.id, undefined);//user.lastContexts);
      });
    },
    // get contexts
    function (userId, lastContexts, callback) {
      exports.getContexts(userId, function (err, contexts) {
        ...
        if (!lastContexts) { // empty string or undefined
          callback(err, userId, contexts[0].id);
        } else {
          callback(err, userId, lastContexts);
        }
      });
    },
...
exports.fullMontyByContext = function (userId, contextId, cb) {
  ...
  async.waterfall([
    ...
    function (callback) {
      exports.getContextById(userId, contextId, function (err, contexts) {
        if (err) {
          callback(err);
          return;
        } else if (_.isEmpty(contexts)) {
