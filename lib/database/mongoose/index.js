...
// });
exports.mainDB = mongoose.connect('mongodb://localhost/test');
...
exports.bootstrap = function (username, cb) {
  ...
  async.auto({
    // get user data
    getUser: function (callback) { // change to getByUserId when sessions are working
...
exports.getContextsByIds = function (userId, contextIds, cb) {
  Context
    .find({ userId: userId, _id: { $in: contextIds } })
    .sort('order')
    .exec(function (err, contexts) {
...
exports.updateUser = function (userId, userModel, cb) {
  ...
  User.findByIdAndUpdate(userId, userModel, cb);
};
