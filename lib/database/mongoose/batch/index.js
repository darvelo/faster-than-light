...
exports.bootstrap = function (user, cb) {
  ...
  function (err, results) {
    ...
    bootstrap.projects = results.getFullMonty.projects;
    bootstrap.auxProjects = results.getFullMonty.auxProjects;
    bootstrap.tasks = results.getFullMonty.tasks;
...
exports.bootstrapByContexts = function (userId, contextIds, cb) {
  var args = _.toArray(arguments);
  ...
  async.auto({
    // get user data
    getUser: function (callback) {
      app.db.users.getUserById(userId, callback);
    },
    // get contexts
    getContexts: ['getUser', function (callback, results) {
      app.db.contexts.getContextsByIds(userId, contextIds, callback);
    }],
    // get projects, auxProjects, and tasks
    getFullMonty: ['getUser', 'getContexts', function (callback, results) {
      exports.fullMontyByContexts(userId, contextIds, callback);
    }],
  },
  function (err, results) {
    if (err) {
    ...
    fullMonty.user = results.getUser;
    fullMonty.contexts = results.getContexts;
    fullMonty.projects = results.getFullMonty.projects;
    fullMonty.auxProjects = results.getFullMonty.auxProjects;
    fullMonty.tasks = results.getFullMonty.tasks;
