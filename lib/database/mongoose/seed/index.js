...
var libSeedTasks = require('./seedTasks.js');
...
exports.seed = function (uname, pw, cb) {
  username = uname;
  password = pw;
  ...
  async.auto( {
    ...
    contexts: ['user', function (callback, results) {
      seedContexts(results.user.id, callback);
    }],
    projects: ['user', 'contexts', function (callback, results) {
      seedProjects(results.user.id, results.contexts, callback);
    }],
    tasks: ['user', 'contexts', 'projects', function (callback, results) {
      seedTasks(results.user.id, results.projects, callback);
    }],
  },
    ...
    checkExist: function (callback) {
      User.find({ username : username }, function(err, results) {
        if (err) {
          ...
          return callback(new e.UserExistsError('Username already exists. Needs to not exist to create stuff.'));
        }

        callback(null, results);
      });
    },
    createHash: ['checkExist', function (callback) {
      app.authentication.hashPassword(password, callback);
    }],
    saveUser: ['createHash', function (callback, results) {
      var user = new User({
        username: username,
        salt: results.createHash[0],
        hash: results.createHash[1],
      });
...
function seedProjects (userId, contexts, cb) {
  ...
  function(err, results) {
    ...
    cb(err, results.saveProjects); // returns an array of saved projects
  });
}
...
function seedTasks (userId, projects, cb) {
  ...
  function generateTasks (callback) {
    var tasks = [];
    var appearancesByTask = libSeedTasks.appearances;

    for (var i = 0; i < libSeedTasks.numTasks; i++) {
  ...
  function modifyAppearances (tasks, appearancesByTask, projects, callback) {
    var rootTasks = []; // array of arrays -- [ [projectId, *rootTasks] ]
  ...
  //    console.log('appearances', appearances, idx);
      var appearancesArray = _.map(appearances, function (appearance) {
        ...
        // populate rootTasks for each project model
        if (object.path === ',') {
          projects[appearance.projectIdx].rootTasks.push(tasks[idx].id);
        }
  ...
  function generateSaveCallback (model) {
    return function (callback) {
      // If you simply model.save(callback), you'll get arguments (err, model, 1)
      // For some reason Mongoose adds a 1 argument to the end.. which will mess up further processing.
      // The solution is to use an anonymous function to callback(err, model) specifically,
      // or to model.save(callback) and elsewhere use underscore's _.flatten() and _.without(1), respectively, on the async.method() results.
  ...
  function saveModifiedCollection (collection, callback) {
    var asyncFuncs = _.map(collection, function (model) {
      return generateSaveCallback(model);
    });

    async.parallel(asyncFuncs, function (err, models) {
      callback(err, models);
    });
  }
  ...
  async.auto({
    ...
    saveProjects: ['linkTasks', function (callback, results) {
      saveModifiedCollection(results.linkTasks[0], callback);
    }],
    saveTasks: ['linkTasks', function (callback, results) {
      saveModifiedCollection(results.linkTasks[1], callback);
    }],
  },
  function (err, results) {
